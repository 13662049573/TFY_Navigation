...
- (void)showAnimationOfProgress:(CGFloat)progress type:(TFY_PageTitleCellAnimationType)type {
    ...
    if (type == TFY_PageTitleCellAnimationTypeSelected) {
        //第一步 颜色渐变
        UIColor *newColor = [TFY_PageControllerUtil colorTransformFrom:self.config.titleSelectedColor to:self.config.titleNormalColor progress:progress];
        ...
        //第一步 颜色渐变
        UIColor *newColor = [TFY_PageControllerUtil colorTransformFrom:self.config.titleNormalColor to:self.config.titleSelectedColor progress:progress];
        [self updateSubviewsColor:newColor];
