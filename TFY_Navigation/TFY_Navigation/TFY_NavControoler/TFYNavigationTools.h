...
@interface TFYNavigationTools : NSObject
...
// -----------------------------------------------------------------------------
typedef NS_ENUM(NSInteger, NavigationSwitchStyle) {
    NavigationSwitchStyleTransition = 0,        // 颜色过渡的方式，支付宝个人中心到余额宝切换效果
    NavigationSwitchStyleFakeNavBar = 1,        // 两种不同颜色导航栏，类似微信红包
};
...
/**
     移动导航栏 / 隐藏系统返回按钮
     [self.navigationController.navigationBar tfy_setTranslationY:(-navBarHeight * progress)];
     [self.navigationController.navigationBar tfy_setBarButtonItemsAlpha:(1 - progress) hasSystemBackIndicator:YES];
 */
